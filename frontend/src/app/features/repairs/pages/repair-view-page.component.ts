        ...
        <section *ngIf="showDetails" class="details-card ui-card" aria-labelledby="repairDetailsTitle">
          ...
          <div class="details-table-wrap">
            <table class="details-table">
              <thead><tr><th>Family</th><th>SN</th><th>MB</th><th>Remark</th></tr></thead>
              <tbody>
                <tr *ngFor="let detail of (item.details?.length ? item.details : [{custsn: '', oldMb: '', family: item.family || '', remark: '', category: item.category || ''}])">
                  <td>{{ detail.family || '—' }}</td><td>{{ detail.custsn || '—' }}</td><td>{{ detail.oldMb || '—' }}</td><td>{{ detail.remark || '—' }}</td>
                </tr>
              </tbody>
            </table>
          </div>
