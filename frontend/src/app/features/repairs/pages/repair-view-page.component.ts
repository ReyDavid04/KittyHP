      ...
      <section class="report-content" *ngIf="!isLoading && repair as item">
        <div class="summary-grid">
          ...
          <article class="detail-card wide">
            <span>Top issue</span>
            <strong>{{ item.topIssue || '—' }}</strong>
          </article>

          <article class="detail-card number-card">
            <span>Failure qty</span>
            <strong>{{ item.failureQty | number }}</strong>
          </article>

          <article class="detail-card number-card">
            <span>Build qty</span>
            <strong>{{ item.buildQty | number }}</strong>
          </article>

          <article class="detail-card number-card">
            <span>F/R</span>
            <strong>{{ item.frPercentage | number: '1.2-2' }}%</strong>
          </article>
          ...
          <article class="detail-card">
            ...
            <strong>{{ item.majorPart || '—' }}</strong>
          </article>

          <article class="detail-card">
            <span>Failure factor</span>
            <strong>{{ item.failureFactor || '—' }}</strong>
          </article>
        </div>
        ...
        <div class="image-grid">
          <article class="image-card">
            ...
            <a *ngIf="item.failPicture; else noFailPicture" [href]="item.failPicture" target="_blank" rel="noopener">
              <img [src]="item.failPicture" alt="Imagen de la falla">
              <span class="zoom-hint">Abrir imagen</span>
            </a>
          ...
          <article class="image-card">
            ...
            <a *ngIf="item.evidencePicture; else noEvidencePicture" [href]="item.evidencePicture" target="_blank" rel="noopener">
              <img [src]="item.evidencePicture" alt="Evidencia de la reparación">
              <span class="zoom-hint">Abrir imagen</span>
            </a>
...
export class RepairViewPageComponent {
  ...
  constructor() {
    ...
    this.repairReportsApi.getOne(repairId).subscribe({
      ...
      error: () => {
        this.isLoading = false;
        this.errorMessage = 'El registro no existe o no está disponible.';
      },
    });
  }
